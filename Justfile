...
initramfs $IMAGE: init-work
    ...
    PATH=/tmp/bin:$PATH dracut --zstd --reproducible --no-hostonly --add "dmsquash-live dmsquash-live-autooverlay" --force /tmp/test/{{ workdir }}/initramfs.img'
...
squash $IMAGE: init-work
    ...
    mksquashfs /rootfs /tmp/test/{{ workdir }}/squashfs.img -all-root"
